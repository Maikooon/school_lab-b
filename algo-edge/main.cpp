...
bool authenticate_move(const RandomWalker& rwer, int current_node, int next_node, int next_community, string VERIFY_SECRET_KEY, std::string& graph_name)
{
...
vector<int> random_walk(std::vector<std::string> table, int& total_move, int start_node, double ALPHA, const RandomWalker& rwer, string graph_name)
{
    ...
    while ((double)rand() / RAND_MAX > ALPHA)
    {
        /*
            現在のノードから次のHop先を探す
            その前に現在のノードがつながっているかつ、アクセス権のあるノードを探す

            配列に格納し、その中からランダムに選択する
        */

        // 読み込んだ行を使って許可されたノードを検索

        std::vector<int> allowed_nodes = search_now_node_is_allowed(table, current_node, start_node);
        std::cout << "allowed_nodes: " << allowed_nodes.size() << std::endl;
        // for (size_t i = 0; i < allowed_nodes.size(); ++i) {
        //     std::cout << allowed_nodes[i]; // 各ノードを出力
        //     if (i < allowed_nodes.size() - 1) {
        //         std::cout << ", "; // 最後の要素でない場合はカンマを追加
        //     }
        // }
        ...
        // 許可されたノードの中から、次のノードをランダムに選択
        int next_node = *next(allowed_nodes.begin(), rand() % allowed_nodes.size());
        ////ここまでを変更すればいいのでは

        // コミュニティが異なる場合には  今のユーザ(コミュニティ)　！＝　次のユーザ(コミュニティ)
        if (node_communities[current_node] != node_communities[next_node])
        {
            ...
            if (!authenticate_move(rwer, current_node, next_node, next_community, VERIFY_SECRET_KEY, graph_name))
            {
...
{// 許可ノードのリストを初期化
    ...
    //ここでパスを指定して、テーブルを読み込む
    std::string extracted = graph_name.substr(0, graph_name.size() - 3); // "cmu"を抽出
    std::string table_path = "./allow-table/" + extracted + ".txt";  //から後ろの三文字を抜き取ったもの
    std::cout << "table path: " << table_path << std::endl;
    //このパスを渡しててテーブルを読み込む 
    std::vector<std::string> table = read_file_lines(table_path);
    ...
    for (const auto& node_entry : graph)
    {
        ...
        // ここでテーブルを読み込んだ変数を渡す
        vector<int> path = random_walk(table, total_move, start_node, ALPHA, rwer, graph_name);
