...
// 認証の関数
bool authenticate_move(int current_node, int next_node)
{
    // ここで異なるコミュニティ間の移動を許可するかどうかの判定を行う
    // 例えば、異なるサーバー間の認証が通るかどうかの判定を行う
    // ここでは単純に true を返す例を示します
    return true; // 仮にすべての移動を許可する場合、true を返す
}
