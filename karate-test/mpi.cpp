// //  MPI用いて、Mainを分散処理をできるようにしたもの
// // ここでは、MPIで出力が期待されない結果になったので、#include <iostream>
// #include <fstream>
// #include <sstream>
// #include <vector>
...
// void personalized_pagerank_with_checks(int rank, int size, const std::unordered_map<int, std::vector<int>> &G, const std::unordered_map<int, std::unordered_set<int>> &communities, double alpha = 0.85, int max_steps = 100)
// {
...
//         {
//             int previous_community = current_community;
//             std::tie(current_node, current_community) = perform_check_and_walk(current_node, current_community, communities, G);

//             // Only print when the community changes
//             if (current_community != previous_community)
//             {
//                 std::cout << "Rwer move from community " << previous_community << " to community " << current_community << std::endl;
//             }
