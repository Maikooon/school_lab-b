...
void RandomWalkSystemWorker::receiveMessage(const uint16_t &port_num)
{
    ...
    while (1)
    {
...

        {
            ...
            for (int i = 0; i < RWer_count; i++)
            {
                if (idx + TOKEN_SIZE > received_bytes)
                {
                    std::cerr << "Error: Token extraction exceeds message size. idx: " << idx << std::endl;
                    break;
                }
                ...
                // RandomWalker のデータを読み取る前にチェック
                size_t rwer_size = sizeof(RandomWalker); // 正確なサイズを使用
                if (idx + rwer_size > received_bytes)
                {
