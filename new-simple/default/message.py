...
# message.py
class Message:
    def __init__(self, ip, next_id, across_server, jwt, end_flag, public_key=None):
        self.ip = ip
        self.next_id = next_id
        self.across_server = across_server
        self.public_key = public_key
    ...
    #     parts = string.split(" ")
    #     self.ip = parts[0]
    #     self.next_id = parts[1]
    #     self.across_server = int(parts[2])
    #     self.jwt = parts[3]
    #     self.end_flag = bool(parts[4])
    @classmethod
    def from_string(cls, string):
        ...
        return cls(
            ip=parts[0],
            next_id=parts[1],
            across_server=int(parts[2]),
            jwt=parts[3],
            end_flag=(parts[4].lower() == "true"),
        )

    def to_string(self):
        return (
            f"{self.ip} {self.next_id} {self.across_server} {self.jwt} {self.end_flag}"
        )

    def update(self, next_ip, next_node, across_server):
        self.ip = next_ip
        self.next_id = next_node
        self.across_server = across_server
