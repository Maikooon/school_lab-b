...
def plot_execution_times(data1, data2, data3, data4):
    ...
    for i in range(count):
        ...
        fig, ax = plt.subplots()
        ax.bar(
            ["no jwt", "default(no cache)", "list", "hash"],
            [
                times1,
                times2,
                times3,
...
# 　ここで読み込むファイルを設定する
def main():
    file1_path = "./certification/construction/nojwt-result/nojwt-result-0.15/overall_average_results.txt"
    ...
    file4_path = "./cache/result/hash/overall_average_results.txt"
