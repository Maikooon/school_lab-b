...
// トークンの生成
std::string generate_token(int proc_rank, int expiration_seconds, int id, const std::string& SECRET_KEY)
{
...
// 認証情報を検証する関数
bool authenticate_move(const std::string& token, int next_node, int proc_rank, string VERIFY_SECRET_KEY)
{
    /// 受け取ったTOkenを出力
...
vector<int> random_walk(int& total_move, int start_node, double ALPHA, int proc_rank)
{
    ...
    // 遷移確立が収束するまで繰り返す、一ループは一歩に当たる
    while ((double)rand() / RAND_MAX > ALPHA)
    {
        //１遷移に限定的なIDをつける　　ここでは限定的に１
        ...
        //次のノードが決まったらトークンを生成
        std::string token = generate_token(proc_rank, expiration_seconds, id, SECRET_KEY);
